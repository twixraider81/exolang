...
if [[ "$LLVM" == "1" && ! -f "$BINDIR/bin/llvm-config" ]]; then
	...
	cmake -G Ninja ../llvm -DCMAKE_INSTALL_PREFIX="$BINDIR" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWITH_POLLY=ON -DLLVM_ENABLE_CXX1Y=ON -DLLVM_ENABLE_ASSERTIONS=ON -DCOMPILER_RT_DEBUG=ON -DLLVM_ENABLE_BACKTRACES=ON -DLLVM_ENABLE_TERMINFO=ON -DLLVM_ENABLE_FFI=ON -DLLVM_INSTALL_UTILS=ON -DLLVM_ENABLE_RTTI=ON
	cmake --build .
