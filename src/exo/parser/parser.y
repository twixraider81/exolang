...
vardecllist ::= vardecllist(l) S_COMMA vardecl(d). {
	BOOST_LOG_TRIVIAL(trace) << "vardecllist ::= vardecllist(l) S_COMMA vardecl(d).";
	POINTERCHECK( l );
	POINTERCHECK( d );
	l->list.push_back( d );
	BOOST_LOG_TRIVIAL(trace) << "Pushing variable declaration; size:" << l->list.size();
...
exprlist ::= exprlist(l) S_COMMA expression(e). {
	BOOST_LOG_TRIVIAL(trace) << "exprlist ::= exprlist(l) S_COMMA expression(e).";
	POINTERCHECK( l );
	POINTERCHECK( e );
	l->list.push_back( e );
...
expression(a) ::= T_VAR(v) S_ASSIGN expression(e). {
	BOOST_LOG_TRIVIAL(trace) << "expression(a) ::= T_VAR(v) S_ASSIGN expression(e).";
	POINTERCHECK( v );
	POINTERCHECK( e );
