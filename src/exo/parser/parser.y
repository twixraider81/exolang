...
program ::= stmts(s). {
	BOOST_LOG_TRIVIAL(trace) << "program ::= stmts(S).";
	ast->stmts = s;
}
...
stmts(s) ::= stmts(a) stmt(b). {
	BOOST_LOG_TRIVIAL(trace) << "stmts(S) ::= stmts(A) stmt(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	a->list.push_back( b );
	s = a;
...
stmt(s) ::= funproto(f) T_SEMICOLON. {
	BOOST_LOG_TRIVIAL(trace) << "stmt(S) ::= funproto(F) T_SEMICOLON.";
	POINTERCHECK(f);
	s = f;
}
stmt(s) ::= fundec(f) T_SEMICOLON. {
	BOOST_LOG_TRIVIAL(trace) << "stmt(S) ::= fundec(F) T_SEMICOLON.";
	POINTERCHECK(f);
	s = f;
}
stmt(s) ::= classdec(c) T_SEMICOLON. {
	BOOST_LOG_TRIVIAL(trace) << "stmt(S) ::= classdec(C) T_SEMICOLON.";
	POINTERCHECK(c);
	s = c;
}
stmt(s) ::= T_RETURN expr(e) T_SEMICOLON. {
	BOOST_LOG_TRIVIAL(trace) << "stmt(S) ::= T_RETURN expr(E) T_SEMICOLON.";
	POINTERCHECK(e);
...
block(b) ::= T_LBRACKET stmts(s) T_RBRACKET. {
	BOOST_LOG_TRIVIAL(trace) << "block(B) ::= T_LBRACKET stmts(S) T_RBRACKET.";
	POINTERCHECK(s);
	b = s;
}
...
stmtif(i) ::= T_IF T_LANGLE expr(e) T_RANGLE block(t) T_ELSE block(f). {
	BOOST_LOG_TRIVIAL(trace) << "stmtif(I) ::= T_IF T_LANGLE expr(E) T_RANGLE block(T) T_ELSE block(F).";
	POINTERCHECK(e);
	POINTERCHECK(t);
	POINTERCHECK(f);
...
vardec(d) ::= type(t) S_VAR(v) T_ASSIGN expr(e). {
	BOOST_LOG_TRIVIAL(trace) << "vardec(D) ::= type(T) S_VAR(V) T_ASSIGN expr(E).";
	POINTERCHECK(t);
	POINTERCHECK(v);
	POINTERCHECK(e);
...
funproto(f) ::= type(t) T_FUNCTION S_ID(i) T_LANGLE vardeclist(l) T_VARG T_RANGLE. {
	BOOST_LOG_TRIVIAL(trace) << "funproto(F) ::= type(T) T_FUNCTION S_ID(I) T_LANGLE vardeclist(L) T_VARG T_RANGLE.";
	POINTERCHECK(t);
	POINTERCHECK(i);
	POINTERCHECK(l);
...
fundec(f) ::= type(t) T_FUNCTION S_ID(i) T_LANGLE vardeclist(l) T_VARG T_RANGLE block(b). {
	BOOST_LOG_TRIVIAL(trace) << "fundec(F) ::= type(T) T_FUNCTION S_ID(I) T_LANGLE vardeclist(L) T_VARG T_RANGLE block(B).";
	POINTERCHECK(t);
	POINTERCHECK(i);
	POINTERCHECK(l);
	POINTERCHECK(b);
...
classdec(c) ::= T_CLASS S_ID(i) T_EXTENDS S_ID(p) T_LBRACKET T_RBRACKET. {
	BOOST_LOG_TRIVIAL(trace) << "classdec(C) ::= T_CLASS S_ID(I) T_EXTENDS S_ID(P) T_LBRACKET T_RBRACKET.";
	POINTERCHECK(i);
	POINTERCHECK(p);
...
classdec(c) ::= T_CLASS S_ID(i) T_LBRACKET classblock(b) T_RBRACKET. {
	BOOST_LOG_TRIVIAL(trace) << "classdec(C) ::= T_CLASS S_ID(I) T_LBRACKET classblock(B) T_RBRACKET.";
	POINTERCHECK(i);
	POINTERCHECK(b);
...
classdec(c) ::= T_CLASS S_ID(i) T_LBRACKET T_RBRACKET. {
	BOOST_LOG_TRIVIAL(trace) << "classdec(C) ::= T_CLASS S_ID(I) T_LBRACKET T_RBRACKET.";
	POINTERCHECK(i);
...
expr(e) ::= constant(c). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= constant(C).";
	POINTERCHECK(c);
	e = c;
}
/* binary ops */
expr(e) ::= expr(a) T_PLUS expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_PLUS expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryAdd( a, b );
}
expr(e) ::= expr(a) T_MINUS expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_MINUS expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinarySub( a, b );
}
expr(e) ::= expr(a) T_MUL expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_MUL expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryMul( a, b );
}
expr(e) ::= expr(a) T_DIV expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_DIV expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryDiv( a, b );
}
expr(e) ::= expr(a) T_EQ expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_EQ expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryEq( a, b );
}
expr(e) ::= expr(a) T_NE expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_NE expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryNeq( a, b );
}
expr(e) ::= expr(a) T_LT expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_LT expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryLt( a, b );
}
expr(e) ::= expr(a) T_LE expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_LE expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryLe( a, b );
}
expr(e) ::= expr(a) T_GT expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_GT expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryGt( a, b );
}
expr(e) ::= expr(a) T_GE expr(b). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(A) T_GE expr(B).";
	POINTERCHECK(a);
	POINTERCHECK(b);
	e = new exo::ast::OpBinaryGe( a, b );
}
expr(e) ::= expr(n) T_ASSIGN expr(v). {
	BOOST_LOG_TRIVIAL(trace) << "expr(E) ::= expr(N) T_ASSIGN expr(V).";
	POINTERCHECK(n);
	POINTERCHECK(v);
